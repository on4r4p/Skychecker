		...
		if (((attributes->vendor_id == 0x12ba) || (attributes->vendor_id == 0x54c)) || (attributes->vendor_id == 0x1430))
		{
			if ((attributes->product_id == 0x150) || (attributes->product_id == 0x967))
			{
				printf("Found portal usb device\n");
                    printf("Vendor ID: %x, Product ID: %x\n", attributes->vendor_id, attributes->product_id);
				int err;
				...
				if (hPortalHandle == NULL)
				{
                    ...
                    /*
                    printf("Manual opening:\n");

                    hPortalHandle = hid_open(0x1430, 0x0150, NULL);
                    if (hPortalHandle == NULL)
                    {
                         printf ("Manual Error communicating with Portal.\n ");
                         printf("Manual Error: %ls\n", hid_error(hPortalHandle));
                    }
                    */
...
#if DEBUG
	printf(">>> PortalIO::CheckResponse from hidapi\n");
#endif
