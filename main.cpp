	...
	if (portalIn && portalOut) {
		...
		if (portalIn) {
			...
			skio->initWithPortal(skylander_number);
               printf("\nAprès skio->initWithPortal(skylander_number)\n");
		}
		if (inFile) {
			if (decrypt) {
          ...
          printf ("\nAvant skio->getSkylander()->validateChecksum()\n");
		if(! skio->getSkylander()->validateChecksum()) {
			printf("Warning. Skylander data read from portal, but checksums are incorrect.  File may be corrupt.\n");
		} 
